...
*** Compiling in Emacs
=M-x compile= runs a compilation command (by default ~make~).  You can
run any command or script, however, and track the output.
...
Emacs will run through a call stack or compiler error log on command.
