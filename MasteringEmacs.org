...
- ~comment-dwim~ (=M-;=) ::
  ...
  or uncomment the region instead.
  ...
  it is defined, otherwise insert a comment and indent it.

  - ARG (=C-u M-;=) :: Else if a prefix ARG is specified, call [[comment-kill][~comment-kill~]].

  Else, call [[comment-indent][~comment-indent~]].
  #+end_defun
...
- ~comment-region~ (=C-c C-c=)::
  ...
  Add comment delimiters to all the lines in the region.  Use =C-c C-c= in
  C-like languages.
  ...
  The strings used as comment starts are built from ~comment-start~ and
