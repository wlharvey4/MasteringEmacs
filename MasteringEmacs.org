...
may also be buffers that are not visiting files.  Each buffer, including the
current buffer, may or may not be displayed in any windows.

#+texinfo:@heading Current Buffer

#+cindex:current buffer
Only one buffer is designated the [[*Current Buffer][current buffer]] at any time.
...
Use ~set-visited-file-name~ to change this variable's value.
#+end_defvar
