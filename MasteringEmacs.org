...
#+author:Pinecone062
#+date:2021-01-19 07:16
#+macro:version Version 0.0.167
...
- function ~unwind-protect~, cleanup expressions
- function ~condition-case~, recover control
...
#+texinfo: @heading The ~signal~ Function
...
=error-message= property of =error-symbol=. The  number and significance of the
objects in =data= depends on =error-symbol=.

- error-symbol

  The argument =error-symbol= must be an /error symbol/---a symbol defined with
  ~define-error~.  An =error  symbol= is  a symbol  with an  ‘error-conditions’
...
#+texinfo: @heading The ~user-error~ Function
...
#+texinfo: @subsubheading Concepts
...
execution of =PROTECTED-FORM=.  Each of the =HANDLER=  s is a list  of the form
~(conditions body…)~.
...
- You can  also specify that a  particular handler should let  the debugger run
  ...
  #+caption:Handle Error on File Deletion
  #+name:handle-error-on-file-deletion
  #+begin_src elisp
    (condition-case nil
	(delete-file filename)
      ((debug error) nil))
...
#+texinfo: @subsubheading Cleaning Up Concept
