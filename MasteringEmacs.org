...
#+cindex:cons cell
#+cindex:CAR, CDR
Lists are built up from /cons cells/, which is a primitive data type in Lisp.
A /cons cell/ is a data object that represents an ordered pair.  That is, it
has two slots, and each slot “holds”, or “refers to”, some Lisp object.  One
slot is known as the CAR, and the other is known as the CDR.

A list is a series of cons cells chained together, so that each cell refers to
the next one.  There is one cons cell for each element of the list.
...
#+header: :eval yes
#+begin_src emacs-lisp
  (if (not (string= file_version "v0.0.0/"))
      nil
    (split-string-and-unquote 
     (shell-command-to-string
      (concat "aws2 s3 ls s3://"
