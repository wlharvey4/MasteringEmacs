...
The operating system groups files into directories.  To specify a file[fn::I
would call this a /path/], you must specify the directory and the file's name
within that directory.

Therefore, Emacs considers a file name[fn:path:path] as having two main parts:

#+cindex:directory name
- the /directory name/ part, and 

- the nondirectory part (or file name within the directory).


Either part may be empty. Concatenating these two parts reproduces the original
file name[fn:path].  On most systems, the directory part is everything up to
and including the last slash; the nondirectory part is the rest.

*** Absolute and Relative File Names---Expanding File Names---Quoting

#+texinfo:@heading Absolute File Names

A file name can specify all the directory names starting from the root of the
tree.  An absolute file name starts with a ‘/’.

#+texinfo:@heading Relative File Names

A file name can specify the position of the file in the tree relative to a
default directory.  A relative file name does NOT start with a ‘/’.
...
- [[*file-name-nondirectory][~file-name-nondirectory~]] --- the file name of a path (everything after last
  slash)

- [[*file-name-extension][~file-name-extension~]] --- return a file name's extension

- [[*file-name-sans-extension][~file-name-sans-extension~]] --- return a file name without its extension

- [[*directory-file-name][~directory-file-name~]] --- a directory file name (without a trailing slash)

- [[*file-name-as-directory][~file-name-as-directory~]] --- given a file, return its directory name (with a
  slash)

- [[*directory-name-p][~directory-name-p~]] --- returns true for directory names

- [[*file-name-absolute-p][~file-name-absolute-p~]] --- returns true if the file name is absolute

- [[*file-relative-name][~file-relative-name~]] --- returns a relative file name in relation to a
  directory

- [[*expand-file-name][~expand-file-name~]] --- expands to absolute an abbreviated file name

- [[*substitute-in-file-name][~substitute-in-file-name~]] --- expands environment variables in a path
...
/directory name/ but is typically not identical.

#+texinfo:@heading The Difference

These two names for the same entity are related by a syntactic transformation:

- On GNU and other POSIX-like systems, this is simple: to obtain a directory
  name, append a ‘/’ to a directory file name that does not already end in ‘/’.

  : directory name = directory file name + /


The difference between a directory name and a directory file name is subtle but
crucial:

- When an Emacs variable or function argument is described as being a directory
  name, a directory file name is not acceptable.

- When [[*file-name-directory][~file-name-directory~]] returns a string, that is always a directory name.

**** file-name-directory

#+attr_texinfo: :options file-name-directory filename
#+begin_defun
- FILENAME :: a file name to process; obtain its directory name;

- RETURN VALUE :: return the directory component in the file name, or =nil= if
  the file name does not include a directory.  Given a Unix syntax file name,
...
- RETURN VALUE :: a file name without an extension;
#+end_defun
...
#+begin_defun
- FILENAME :: a file name upon which to operate, defaulting to
