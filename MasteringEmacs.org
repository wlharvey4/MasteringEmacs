...
#+findex:delq
#+attr_texinfo: :option delq object list
#+begin_defun
...
- =M-x run-lisp= ::
  ...
  To begin an external Lisp session, type =M-x run-lisp=. This runs
  the program named =lisp=, and sets it up so that both input and
  output go through an Emacs buffer named =*inferior-lisp*=. This
  command also switches back to an existing inferior lisp job. To
  change the name of the Lisp program run by ‘M-x run-lisp’, change
  the variable ‘inferior-lisp-program’.
