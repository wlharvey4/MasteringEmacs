...
events that is meaningful  as a unit.  If a key sequence  invokes a command, it
is called  a {{{dfn(complete key)}}}.  If  a key sequence isn’t  long enough to
invoke a command, it is called  a {{{dfn(prefix key)}}}.  Every key sequence is
